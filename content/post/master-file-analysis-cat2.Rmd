---
title: 'Master file Analyis - cat 2'
author: |
...
dyn_ae_ge_uniq_fam <- dyn_ae_ge%>% filter(dyn_tot_2 ==1) %>% distinct(family_id, .keep_all = TRUE)
...
This universe includes winners and runner-ups in the general and assembly elections from the year 1977 to 2019. This contains `r dim(dyn_ae_ge)[1]` observations and the number  unique individuals is `r dim(dyn_ae_ge_uniq_ind)[1]`  and the families  is  `r (dim(dyn_ae_ge_uniq %>% filter(dyn_tot_2==1))[1])`. It has to be noted that `r scales::percent(dim(dyn_ae_ge %>% filter (relative_id ==1))[1]/dim(dyn_ae_ge)[1])` of this winners are just patriarchs.

In this universe of families `r dim(dyn_ae_ge %>% filter(dyn_tot_2==1))[1]` winners and runner-ups are part of some family and `r dim(dyn_ae_ge %>% filter(dyn_tot_2==1 & relative_id ==1))[1]` winners and runner-ups out of this is just patriarchs while the rest is other family members. 
...
# dyn_ae_ge_fam %>% filter(family_id == "f0030811")
# 
# 
# dyn_ae_ge_fam_uniq  %>% filter(life_span ==0)
...
ggplot(dyn_ae_ge_fam_uniq)+
  ...
  # geom_point(x = medx, y = 0, color = "red")+
  # geom_point(x = meanx, y = 0, color = "blue")+
...
ggplot(dyn_ae_ge_uniq)+
  ...
  # # geom_vline(xintercept = medx, color = "red")+
  # # geom_vline(xintercept = meanx, color = "blue")+
  # geom_text(aes(x=(medx-.8), label="Median", y=.1), colour="red", size =3) +
  # geom_text(aes(x=(meanx+.8), label="Mean", y = .1), colour="blue", size = 3)+
  theme_minimal_hgrid()+
...
# x.dens <- density(dyn_ae_ge_fam_uniq$n_el_c_fam)
# df.dens <- data.frame(x = x.dens$x, y = x.dens$y)
# p + geom_area(data = df.dens, 
...
#   geom_vline(intercept = dyn_ae_ge_fam_uniq$n_el_c_fam)
#   geom_vline(data=df, aes(yintercept = median), colour=“red”) 
