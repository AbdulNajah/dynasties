...
fit_rural  <- glm(dyn_duration ~ pc91_vd_power_supl + p_sch_r91+ +irr_share91 +hosp_r91  + pc91_vd_app_pr + constituency_type +Prural,data = dyn_asher, family=poisson())

fit_urban <- glm(dyn_duration ~  el_con91+ p_sch_u91 +s_sch_u91 +irr_share91+ hosp_u91 + pc91u_td_p_road + constituency_type+p_urban,data = dyn_asher, family=poisson())

#summary(fit.1)

stargazer(fit_all , fit_urban, fit_rural,title="Regression Results - Dev Asher ",type = "html",
align=TRUE ,dep.var.labels= c ("Urban + Rural", "Urban", "Rural"), no.space=TRUE)
