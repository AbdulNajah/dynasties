...
dyn_cabinet <-  cabinet  %>% filter(year!= "" & family_id >0 & family_id != "UK") %>% group_by(family_id) %>%summarise(n_sc_minster = n(), minister_dur = sum(term_months, na.rm = TRUE))
...
fit <- glm(dynast ~ N_minister+minister_duration, family =  binomial(link = "probit"))

#summary(fit)

stargazer(fit,title="Regression Results",type = "html",
 dep.var.labels=c("Dynast - 1/0"),
