...
dyn %>% filter(position==1)%>% group_by(election_type,contest_type) %>% summarise(mean(margin_percentage))  %>% kable(caption = "Margin of victory wrt contest type", col.names = c("Election Type", "contest type", "Average MOV percentage"), digit = 1) %>% kable_styling(bootstrap_options = "striped")
