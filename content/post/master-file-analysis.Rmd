...
ae_dyn <- dyn_ae%>%  group_by(year, dyn_tot) %>% summarise(count = n_distinct(family_id))



ae_dyn <- ae_dyn %>% spread(key = dyn_tot, value = count ) %>%  rename( non_dyn= "0", dyn = "1") ##%>% mutate(prop = (dyn/non_dyn))
