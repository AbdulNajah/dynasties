...
dyn_ae_ge_uniq_fam <- dyn_ae_ge_uniq %>% filter(dyn_tot ==1)
total_count <-dyn_ae_ge_uniq_fam %>% filter(relative_id !="NA") %>% count() %>% as.integer()

dyn_ae_ge_uniq_fam %>% filter(relative_id !="NA") %>% group_by( relative_id,relative_name) %>% summarise(count = n(), prop = count/total_count) %>%  kable(Caption = "Members distribution among families \n AE-GE",col.names = c("Relative ID","Relative", "Count", "Proportion"), digits =2) %>% kable_styling(bootstrap_options = "striped")
...
dyn_rlb <- dyn %>% filter(election_type %in% c("ZP", "BP"))


dyn_rlb <- dyn_rlb %>% group_by(family_id)%>%  mutate(min_year = min(year), max_year = max(year))

dyn_bp <- dyn_rlb %>% filter(election_type == "BP")

#dyn_bp <- dyn_bp %>% group_by(family_id)%>%  mutate(min_year = min(year), max_year = max(year))


dyn_bp %>%distinct(family_id, .keep_all = TRUE) %>%  group_by(min_year, max_year) %>% summarise(count = n()) %>% group_by(min_year) %>% arrange(min_year, -max_year) %>%  mutate(min_yr_tot = sum(count),cum_sum =  cumsum( count), prop = cum_sum/min_yr_tot ) %>% 
  ggplot(aes(factor(max_year), prop, group = min_year, color = factor(min_year)))+
  geom_point()+
  geom_line()+
  theme_minimal()+
  scale_y_continuous(breaks = c(0,.2,.4,.6,.8,1), limits = c(0,1))+
  labs(title = "Candidates/family retention in BP elections", x = "Year", y = "Proportion")+
...
#prop_ind <- read.csv("D:/cpr/up-dynasties/dyn_other_data/dyn_prop_ind.csv")
...
dyn_ulb <- dyn_ulb %>% group_by(family_id)%>%  mutate(min_year = min(year), max_year = max(year))

dyn_np_nnp <- dyn_ulb %>% filter(election_type %in% c("NP", "NNP"))


dyn_np_nnp %>%distinct(family_id, .keep_all = TRUE) %>%  group_by(min_year, max_year) %>% summarise(count = n()) %>% group_by(min_year) %>% arrange(min_year, -max_year) %>%  mutate(min_yr_tot = sum(count),cum_sum =  cumsum( count), prop = cum_sum/min_yr_tot ) %>% 
  ggplot(aes(factor(max_year), prop, group = min_year, color = factor(min_year)))+
  geom_point()+
  geom_line()+
  theme_minimal()+
  labs(title = "Candidates/family retention in NP &NPP elections", x = "Year", y = "Proportion")+
  scale_y_continuous(breaks = c(0,.2,.4,.6,.8,1), limits = c(0,1))+
...
dyn_ae_ge_uniq <-  dyn_all %>% filter(election_type %in% c("AE", "GE")) %>% arrange(-year) %>%
  distinct( family_id, .keep_all = T)
dyn_ae_ge_uniq  %>% filter(ind_2 != 0) %>% group_by(dyn_tot_text, ind_2) %>% summarise(count = n()) %>% group_by(dyn_tot_text) %>% mutate(sum = sum(count), prop = count/sum) %>% select(dyn_tot_text, ind_2, prop) %>% 
