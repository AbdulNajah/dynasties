...
dyn_pmgsy$dyn_cum_text <- ifelse(dyn_pmgsy$dyn_cum ==0, "Non-family","Family")
...
#  dyn_pmgsy  %>% group_by(dyn_cum_text) %>% summarise(mean_proj = mean(ac_proj_no), mean_cost = mean(sanc_cost_rup ), mean_rd_len = mean(`road length (kms)`))
# 
...
#  cbind(Entity,dyn_pmgsy_mean) %>% select(-dyn_cum) %>%  kable(caption = "PMGSY summary wrt constituency type", col.names = c ("Type of constituency", "Average number of projects","Avergae expenditure per project", "Average road length (km)"),digits = 1)%>% kable_styling(bootstrap_options = "striped")
 ...
 dyn_pmgsy %>% group_by(dyn_cum_text) %>% summarise(sum = sum(`road length (kms)`), count = n_distinct(constituency_no),mean_proj_con = n()/count,mean_proj = mean(`road length (kms)`), mean_con = sum/count) %>% select(dyn_cum_text,mean_proj_con,mean_proj, mean_con) %>% kable(caption = "PMGSY summary stats wrt constituency type", col.names = c("Constituency Type","Average number of projects"," Average length per project", "Average length over all"),digits = 1)%>% kable_styling(bootstrap_options = "striped")
...
dyn_pmgsy %>% group_by(dyn_cum_text) %>% summarise(mean(n_cont_ind),sum(sanctionedcostnew)/n_distinct(`contractor name`)) %>% 
...
# con_caste_1 <- dyn_pmgsy %>% group_by(dyn_cum,caste_groups) %>% summarise(cont =n()/n_distinct(`contractor name`))
# 
# con_caste_2 <- dyn_pmgsy %>% group_by(caste_groups) %>% summarise(n()/n_distinct(cont = `contractor name`)) %>% mutate(dyn_cum = .5)
# 
# con_caste_3 <- rbind(con_caste_1,con_caste_2)
#  ggplot(con_caste_3 %>% filter(caste_groups!= "Others") ,aes(factor(reorder(caste_groups,cont)),cont, fill = factor(dyn_cum)))+
...
dyn_pwd$dyn_cum_text <- ifelse(dyn_pwd$dyn_cum ==0, "Non-family","Family")
...
# 
# dyn_pwd  <-
#   dyn_pwd  %>% group_by(constituency_no) %>% mutate(ac_proj_no = n())
# 
# 
...
```{r}
# dyn_pwd %>% group_by(fam_exp_cum_cat)  %>% summarise( n()/n_distinct(constituency_no)) %>% 
#   kable(caption = "Average number of projects and length of the road wrt constituency type", col.names = c("Constituency Type","Number of projects","Length per project", "Length over all"),digits = 1)%>% kable_styling(bootstrap_options = "striped")
...
```{r}
dyn_pwd %>% group_by(dyn_cum_text,fam_exp_cum_cat) %>% summarise(proj =n()/n_distinct(constituency_no), len = sum(`road length`)/n_distinct(constituency_no)) %>% select(dyn_cum_text,fam_exp_cum_cat,proj, len) %>% kable(caption = "Average number of projects and length of the road wrt experience category", col.names = c("Constituency type","Experience category","Number of projects","Length over all"),digits = 1)%>% kable_styling(bootstrap_options = "striped")
...
```
<!-- ```{r} -->
<!-- dyn_pwd %>% group_by(caste_groups) %>% summarise(proj =n()/n_distinct(constituency_no), len = sum(`road length`)/n_distinct(constituency_no)) %>%  -->
...
# dyn_ge_w %>% group_by(fam_exp_cum_cat_1) %>% summarise(n())
# 
# dyn_ae_w %>% group_by(fam_exp_cum_cat_1) %>% summarise(n())
# 
# dyn_pwd <- dyn_pwd %>% group_by(constituency_no) %>% mutate(rd_len_con = sum(`road length`))
# 
# ggplot(dyn_pwd %>% filter(dyn_cum == 1), aes(fam_exp_cum_cat, rd_len_con))+
#   geom_boxplot()
# 
# 
# dyn<- dyn %>%  mutate(fam_exp_cum_cat_1 = cut(fam_exp_cum,c(0,1,11,21,31,Inf),include.lowest = TRUE, right =FALSE))
# 
# 
# dyn_ae_w <- dyn %>% filter(year == 2017 & election_type == "AE" & position==1)
# 
# dyn_ge_w <- dyn %>% filter(year == 2019 & election_type == "GE" & position==1)
# 
# 
# dyn_pmgsy <- merge(dyn_ge_w,pmgsy, by = c("year", "constituency_no"))
