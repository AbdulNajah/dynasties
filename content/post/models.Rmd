...
dyn_indus <- dyn_indus %>% dplyr::select(election_type, year, pid, indus_rt, indus_nrt,indus_count)

# 
dyn_ae_ge <- merge(dyn_ae_ge, dyn_indus, by = c("election_type", "year", "pid"), all.x = TRUE)
## cabinet
...
#library(glmnet)
fit.2 <- glm(linkage~  caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + post_90 + n_elections_w , family="binomial")

 stargazer(fit.1, fit.2,title="Regression Results - GE All ",type = "html",
align=TRUE, dep.var.labels=c("Dynast","Linkage"), no.space=TRUE)
# 
...
dyn_ge_09_19_uniq  <- dyn_ae_ge %>% filter(year %in% c(2009,2014,2019)) %>% group_by(family_id) %>% arrange(-year) %>% distinct(family_id, .keep_all = TRUE)


dynast <-dyn_ge_09_19_uniq $dyn_tot
...
total_assets <- dyn_ge_09_19_uniq $total_assets
...
fit.1 <- glm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land +n_elections_w+ total_assets  + serious_crime + turncoat, family = binomial(link = "probit"))




fit.2 <- glm(winner ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w + total_assets + serious_crime + turncoat + dynast, family = binomial(link = "probit"))

fit.3 <- lm(vote_share ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w + total_assets  + serious_crime + turncoat + dynast)
...
stargazer(fit.1, fit.2, fit.3,title="Regression Results - AE & GE 2009:19 ",type = "html",
align=TRUE, dep.var.labels=c("Dynast","Winner", "Vote Share Percentage"), no.space=TRUE)
