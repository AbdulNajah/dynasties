...
fit.1 <- glm(dynast ~ caste_uc + link + industry+ minister_duration+decades  , family = binomial(link = "probit"))


position <- ifelse(position ==1,1,0)

fit.2 <- glm(winner ~ vote_share + dynast + caste_uc + incumbent + turnout + industry + minister_duration + decades + link , family = binomial(link = "probit") )
...
stargazer(fit.1, fit.2,fit.3,title="AE all - Regression Results",type = "html",
...
turnout <- dyn_ge_uniq$turnout_percentage
...
position <- ifelse(position ==1,1,0)
fit.1 <- glm(winner ~ vote_share + dynast + caste_uc + incumbent + turnout +turncoat+ industry + link , family = binomial(link = "probit") )
# 
# summary(fit.1)


fit.2 <- lm(vote_share ~ dynast + caste_uc +incumbent + turnout + industry + link+ turncoat)

# summary(fit.2)

stargazer(fit.1, fit.2,title=" GE all - Regression Results",type = "html",
align=TRUE,dep.var.labels=c("Winner","Vote share"),
omit.stat=c("LL","ser","f"), no.space=TRUE)
...
fit.1 <- glm(position ~ caste_uc + vote_share + turnout + industry + incumbent + turncoat + minister_duration + link + movable_assets + immovable_assets + serious_crime + non_serious_crime + dynast)

# summary(fit.1)

fit.2 <- lm(vote_share ~ caste_uc +  turnout + industry + incumbent + turncoat + minister_duration + link + total_assets + serious_crime + non_serious_crime + dynast)
# 
# summary(fit.2)

stargazer(fit.1, fit.2,title="AE - 2012:17 - Regression Results",type = "html",
align=TRUE,dep.var.labels=c("Winner","Vote share"),
omit.stat=c("LL","ser","f"), no.space=TRUE)
...
fit.1 <- glm(winner ~ caste_uc + vote_share+ turnout + industry + incumbent + turncoat + link + serious_crime + non_serious_crime + dynast + total_assets, family = binomial(link = "probit"))

# summary(fit.1)

fit.2 <- lm(vote_share ~ caste_uc + winner + turnout + industry + incumbent + turncoat + link + serious_crime + non_serious_crime + dynast+total_assets)

# summary(fit.2)


stargazer(fit.1, fit.2,title="GE - 2009:19 - Regression Results",type = "html",
align=TRUE,dep.var.labels=c("Winner","Vote share"),
omit.stat=c("LL","ser","f"), no.space=TRUE)
