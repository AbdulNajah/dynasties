...
```{r}
dynast_adr %>% group_by(dynast) %>% summarise(job = mean(`Training for jobs`, na.rm = TRUE), 
   ...
   melt(id = "dynast", measure = 2:11) %>% pivot_wider(names_from = "dynast", values_from = "value") %>%
...
```{r}
 dynast_adr %>% group_by(dynast) %>% summarise(job = mean(`I: Training for jobs`, na.rm = TRUE), 
...
```{r}
dynast_adr %>% group_by(dynast) %>% summarise(job = mean(`P: Training for jobs`, na.rm = TRUE), 
