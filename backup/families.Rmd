...
dyn_ae_ge_fam <- dyn_ae_ge %>% filter(dyn_tot ==1 )
```


```{r}

# dyn_ae_ge_fam %>% filter(position ==1) %>%  group_by(family_id, decade) %>% summarise(count = n()) %>% spread(decade, count) 
# dyn_ae_ge_fam <- dyn_ae_ge_fam %>% group_by(family_id) %>% mutate(max_decade = max(decade)[position==1])
# 
# data.frame(table(dyn_ae_ge_fam_uniq$max_decade)) %>% filter(Var1 != 2010) %>%  kable(caption = "Families decline by decade",digits = 2, col.names = c("Decade", "Count")) %>% kable_styling(bootstrap_options = "striped") 
# 
# prop.table(table(dyn_ae_ge_fam_uniq$max_year)) %>% kable(digits = 2)
# 
# dyn_ae_ge_fam_uniq <-  dyn_ae_ge_fam%>% distinct(family_id, .keep_all = TRUE) #%>%group_by(decade) %>% summarise(n())

```

```{r}
#dyn_ae_ge %>%  filter(dyn_tot ==1 &str_detect(level, "LB") ) %>% distinct(family_id)

# dyn_ae_ge %>% group_by(family_id) %>% filter(year ==min(year) & dyn_tot ==1) %>% filter(election_type %!in% c("AE", "GE"))



# fam_2 %>% select(decade, prop) %>% kable(caption = "Individual to family conversion of all families across decades", digits = 2)



fam_1 <- dyn_ae_ge_fam %>% group_by(family_id) %>% arrange(year) %>%  filter(fam_size_tot >=2 &min(decade)) %>% distinct(family_id, .keep_all = TRUE) %>% group_by(decade) %>% summarise(count = n())# %>% kable()


fam_1$sum <- sum(fam_1$count)

fam_1$prop <- fam_1$count/fam_1$sum



fam_1 <- fam_1 %>% select(decade, prop)

fam_2 <-  dyn_ae_ge_fam %>% group_by(family_id) %>% arrange(year) %>%  filter(fam_size_cum>= 2 &min(decade)) %>% distinct(family_id, .keep_all = TRUE) %>% group_by(decade) %>% summarise(count = n())

fam_2$sum <- sum(fam_2$count)

fam_2$prop <- fam_2$count/fam_2$sum

fam_2 <- fam_2 %>% select( prop) 

cbind(fam_1, fam_2) %>% kable(col.names = c("Decade", "Proporion of families started", "Proportion of families with second member"), digits = 2) %>% kable_styling(bootstrap_options = "striped")

fam_1 %>% select(decade, prop) %>% kable(caption = "Proportion of families started in each decade ", digits = 2)


```

```{r}
fam_pre02 <-  dyn_ae_ge_fam %>% group_by(family_id) %>% filter(dyn_cum ==1 & year<=2007) %>% distinct(family_id)
...
dyn_ae_ge_fam_uniq <- dyn_ae_ge_fam  %>% distinct(family_id, .keep_all = TRUE)
...
fit.1 <- glm(success ~  caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + linkage, family= binomial(link = "probit") )
